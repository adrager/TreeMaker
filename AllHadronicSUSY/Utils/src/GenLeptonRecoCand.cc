...
GenLeptonRecoCand::GenLeptonRecoCand(const edm::ParameterSet& iConfig)
{
	...
	const std::string string1Neu("Neutrino");
	...
	const std::string string4tt2("TauDecayCandspdgID");
        const std::string string4tt3("TauDecayCandsCharge");
	produces<std::vector<reco::GenParticle> > (string1).setBranchAlias(string1);
	produces<std::vector<reco::GenParticle> > (string1Neu).setBranchAlias(string1Neu);
...
GenLeptonRecoCand::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  std::auto_ptr< std::vector<int> > selectedTauDecayCandsCharge(new std::vector<int>);
	...
	for(size_t i=0; i<pruned->size();i++)
	{
		if( (abs((*pruned)[i].pdgId() ) == 24 || abs((*pruned)[i].pdgId() ) == 23 ) && (*pruned)[i].status()==22) // needs to be checked if this workes for Z 23 as well
		{
			...
			for(size_t ii=0;ii< bosonDaugthers; ii++)
			{
				...
				if(abs(FinalBoson->daughter(ii)->pdgId())== 12 || abs(FinalBoson->daughter(ii)->pdgId())== 14)
				{
					selectedNeutrino->push_back(*((reco::GenParticle*) FinalBoson->daughter(ii) ));
// 					std::cout<<"Found neutrino: "<<abs(FinalBoson->daughter(ii)->pdgId())<<"\n";
				}
				if(abs(FinalBoson->daughter(ii)->pdgId())== 15) 
				{
					...
					{
//                                           std::cout<<"Daughter ["<<iii<<"]: "<<FinalTauDecay->daughter(iii)->pdgId()<<"\n";
                                          ...
                                          selectedTauDecayCandsCharge->push_back(FinalTauDecay->daughter(iii)->charge());
					}
					if(FinalTauDecay->numberOfDaughters()==0)
                                        {
                                          std::cout<<"Waring Tau has no daugthers!!!! \n";
                                        }
// 					FinalTauDecay = TauFound((reco::GenParticle*) FinalBoson->daughter(ii));
//                                         for(size_t iii=0; iii<FinalTauDecay->numberOfDaughters();iii++)
//                                         {
//                                           std::cout<<"Daughter ["<<iii<<"]: "<<FinalTauDecay->daughter(iii)->pdgId()<<"\n";
//                                           selectedTauDecayCands->push_back(*((reco::GenParticle*) FinalTauDecay->daughter(iii) ));
...
// 					const reco::GenParticle * FinalTauDecay = TauFound((reco::GenParticle*)FinalBoson->daughter(ii));
					...
					for(size_t iii=0; iii<FinalTauDecay->numberOfDaughters();iii++)
					{
						...
						else if(abs(FinalTauDecay->daughter(iii)->pdgId())== 15)
                                                {
                                                  std::cout<<"Warning tau decay cand num: "<<iii<<" is: pdgID 15\n";
                                                }
...
// 						else std::cout<<"No lep decay tau with daughters["<<iii<<"]: "<<FinalTauDecay->daughter(iii)->pdgId()<<std::endl;
	...
	const std::string string4tt2("TauDecayCandspdgID");
  const std::string string4tt3("TauDecayCandsCharge");
	iEvent.put(selectedNeutrino,string1neu);
	...
	iEvent.put(selectedTauDecayCandspdgID,string4tt2);
        iEvent.put(selectedTauDecayCandsCharge,string4tt3);
