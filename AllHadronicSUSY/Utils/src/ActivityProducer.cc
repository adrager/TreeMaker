...
private:
	...
	edm::InputTag objectSource_, objectMatchSource_, WeightTag_, HTTag_, MHTTag_, NJetsTag_;
...
ActivityProducer::ActivityProducer(const edm::ParameterSet& iConfig)
{
	...
	NJetsTag_ = iConfig.getParameter<edm::InputTag>("NJetsTag");
	...
	produces<edm::ValueMap<float> >("HT");
	produces<edm::ValueMap<float> >("MHT");
	produces<edm::ValueMap<float> >("NJets");
...
ActivityProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	iEvent.getByLabel(NJetsTag_,NJets);
	...
	if(objectTyp_==1)
	{
				...
				if(deltaR(object->at(i).eta(),object->at(i).phi(),objectMatch->at(ii).eta(),objectMatch->at(ii).phi() ) < 0.07&& std::abs(object->at(i).pt() - objectMatch->at(ii).pt() )/objectMatch->at(ii).pt() < 0.4)
				{
					...
					if(objectMatch->size()==2 && objectMatch->size()== tagObject->size())
					{
						...
						else if(ii==1) 
						{
							...
							metLorentz2clean+= objectMatch->at(0).p4();
						}
					}
					else 
					{
		...
		std::auto_ptr<ValueMap<float> > valMap91(new ValueMap<float>());
		ValueMap<float>::Filler filler91(*valMap91);
		filler91.insert(object, HT_.begin(), HT_.end());
		filler91.fill();
		iEvent.put(valMap91, "HT");
		std::auto_ptr<ValueMap<float> > valMap92(new ValueMap<float>());
		ValueMap<float>::Filler filler92(*valMap92);
		filler92.insert(object, MHT_.begin(), MHT_.end());
		filler92.fill();
		iEvent.put(valMap92, "MHT");
		std::auto_ptr<ValueMap<float> > valMap93(new ValueMap<float>());
		ValueMap<float>::Filler filler93(*valMap93);
		filler93.insert(object, NJets_.begin(), NJets_.end());
		filler93.fill();
		iEvent.put(valMap93, "NJets");
